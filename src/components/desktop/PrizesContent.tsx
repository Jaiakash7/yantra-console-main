...
const PrizesContent = () => (
  <div className="h-full w-full overflow-y-auto scrollbar-hide p-10 text-foreground">
    <motion.div
    ...
    <div className="grid grid-cols-3 gap-8 mb-10">
      {categories.map((cat, i) => (
        <motion.div
          key={cat.title}
          className={`bg-zinc-900/60 backdrop-blur-xl rounded-2xl border p-10 flex flex-col h-full ${cat.glowBorder}`}
        ...
        >
          <div className="flex items-center gap-5 mb-8">
            ...
            <div>
              <span className="font-display text-lg tracking-widest text-foreground font-bold block mb-1">{cat.title}</span>
              <span className="font-mono text-sm tracking-[0.2em] text-muted-foreground">{cat.subtitle}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 mt-auto">
            {[
              ...
              <div key={p.label} className={`rounded-xl border p-4 flex flex-col items-center justify-center text-center transition-transform hover:scale-105 ${p.bg}`}>
                <span className="text-4xl block mb-2 drop-shadow-md">{p.emoji}</span>
                <span className="text-[9px] font-mono tracking-widest text-muted-foreground block">{p.label}</span>
                <span className="text-xl font-mono text-primary font-bold block mt-1">{p.amount}</span>
              </div>
            ))}
          </div>
        </motion.div>
